...
namespace Earthwatchers.UI.Layers
{
    public class FincasNameLayer : Layer
    {
        ...
        private List<Basecamp> _fincasRegion;
        ...
        {
            _fincasRegion = _fincas.Where(x => x.RegionId == Current.Instance.Earthwatcher.PlayingRegion).ToList();
            ClearNames();
            //Cargo los puntos de cada finca
            if (_fincasRegion != null)
            {
                foreach (var f in _fincasRegion)
                {
                    ...
                    if (res > 2.79 && res <= 19.21)
                    {
                        ...
                        {
                           symbolStyle = new SymbolStyle { Symbol = GetSymbol("../Images/FincasName/noImage.png"), SymbolType = SymbolType.Rectangle };
                        }
