...
namespace Earthwatchers.UI.Requests
{
    public class LandRequests
    {
        ...
        public event ChangedEventHandler CheckPercentageReceived;
        public event ChangedEventHandler PresicionPercentageReceived;
        ...
        public void GetCheckPercentage(int regionId)
        {
            var request = new RestRequest("land/getCheckPercentage=" + regionId, Method.GET);
            client.ExecuteAsync<decimal>(request, response =>
                Deployment.Current.Dispatcher.BeginInvoke(() =>
                    CheckPercentageReceived(response.Content, null)
                    ));
        }
