...
namespace Earthwatchers.UI.GUI.Controls
{
    public partial class NotificationsWindow
    {
        ...
        public NotificationsWindow(string action)
        {
            ...
            if(action.Equals(ActionPoints.Action.RegionCompleted.ToString()))
            {
                this.Title.Text = Labels.Notifications3;
                this.Body.Text = Labels.NoMoreLands;
                this.ButtonClose.Visibility = System.Windows.Visibility.Visible;
                this.ButtonContinue.Visibility = System.Windows.Visibility.Collapsed;
            }
        }
        /// <summary>
        /// Cambiar parcela cada 5 minutos o libremente si sos admin
        /// </summary>
        ...
        void scoreRequests_ServerDateTimeReceived(object sender, EventArgs e)
        {
            ...
            if (Current.Instance.Earthwatcher.Role == Role.Admin) //Si sos admin siempre te deja cambiar
            {
            ...
            else //si no sos admin evalua las condiciones normales 
            {
                if (Current.Instance.RegionScores.Any(
                    x => x.EarthwatcherId == Current.Instance.Earthwatcher.Id && 
                    x.Action.StartsWith(ActionPoints.Action.LandReassigned.ToString()) &&
                    x.Published > serverTime.AddMinutes(-5))) //TIEMPO EN EL CAMBIO DE PARCELA 
                {
