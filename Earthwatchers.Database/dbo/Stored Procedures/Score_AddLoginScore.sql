...
DECLARE @id INT 
                    SET @id = (Select Id From Earthwatcher Where Name = @name)
                     ...
                     Insert Into scores(EarthwatcherId, action, published, points, RegionId)
                    Select @id, 'Login', GETUTCDATE(), CASE WHEN (
			                    ...
			                    From (
				                    Select TOP 1 published 
									From scores 
									where action = 'Login' and EarthwatcherId = @id and points > 0 
									Order By published DESC) lastlogins
		                    ) = 1 THEN 100 ELSE 0 END,
							(select PlayingRegion from Earthwatcher where Id = @id)
                     From Earthwatcher
					 Where Id = @id
END
