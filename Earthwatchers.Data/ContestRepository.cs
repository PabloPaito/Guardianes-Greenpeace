   ...
   public class ContestRepository : IContestRepository
    {
        ...
        public Contest GetContest(int regionId)
        {
            var all = GetAllContests();

            var contest = all.Where(x => x.StartDate <= DateTime.UtcNow && x.EndDate >= DateTime.UtcNow && x.RegionId == regionId).FirstOrDefault();
            return contest;
        }
