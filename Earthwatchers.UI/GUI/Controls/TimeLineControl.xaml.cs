...
namespace Earthwatchers.UI.GUI.Controls
{
    public partial class TimeLineControl
    {
        ...
        private SatelliteImage forestLaw;
        private List<SatelliteImage> includedSatelliteImages;
        private ImageRequests imageRequests;
        private LayerHelper layerHelper;
        private LayerControlOnOff hexagonOnOff;
        private LayerControlOnOff forestLawOnOff;
        private LayerControlOnOff basecampsOnOff;
        private string title = string.Empty;
        private double sliderOldValue = 0;
        ...
        public ImageRequests ImageRequests
        {
            get
            {
        ...
        {
            imageRequests.GetAllImagery(Current.Instance.Earthwatcher.PlayingRegion);
        }


        //Slider Animations (Tutorial UI)
        ...
        //Satellite Images
        public void UpdateImagesInTimeline(List<SatelliteImage> newImages)
        {
            //this.satelliteImages = newImages;

            ////Updateo las imagenes del combo
            //if (satelliteImages != null)
            //{
            //    List<ILayer> layersToRemove = new List<ILayer>();

            //    layersToRemove.AddRange(Current.Instance.LayerHelper.Layers.Where(x =>
            //       (x.LayerName != Constants.AlertedLandLayername
            //     && x.LayerName != Constants.ArgentineLawlayername
            //     && x.LayerName != Constants.BasecampsLayer
            //     && x.LayerName != Constants.DemandAuthoritiesLayerName
            //     && x.LayerName != Constants.fincasLayerName
            //     && x.LayerName != Constants.Hexagonlayername
            //     && x.LayerName != Constants.jaguarLayerName)).ToList());

            //    foreach (var l in layersToRemove)
            //    {
            //        Current.Instance.LayerHelper.RemoveLayer(l.LayerName);
            //    }
            ...
            //        if (text != null)
            //            ChangingOpacity(this, new ChangingOpacityEventArgs { Title = text.Text, IsCloudy = includedSatelliteImages.FirstOrDefault().IsCloudy, IsInitial = true });
            //    }
            //    this.RefreshMap();
            //}
        }
