...
namespace Earthwatchers.UI.Layers
{
    public class BasecampLayer : Layer
    {
        ...
        {
            var region = Current.Instance.Earthwatcher.PlayingRegion != 0 ? Current.Instance.Earthwatcher.PlayingRegion : 1;
            var layerName = Earthwatchers.Models.KmlModels.Layers.FincasName.GetStringValue() + region.ToString();
            FincasNameLayer fincasLayerNames = Current.Instance.LayerHelper.Layers.Where(x => x.LayerName == "NombreFincas").FirstOrDefault() as FincasNameLayer;
            
            if (isFirstTime || _regionId != region) //Para no agregarlo cada vez
            {
                if(_regionId != region)
                {
                //Saco las fincas de otra region
                    _source.Features.Clear();
                //Regcargo los nombres para la nueva region
                    fincasLayerNames.ReloadNames();
                }

                layerRequest.GetLayersByName(layerName, true);
