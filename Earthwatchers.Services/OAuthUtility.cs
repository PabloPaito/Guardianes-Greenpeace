...
namespace Earthwatchers.Services
{
    public class OAuthUtility
    {
        ...
        public static string GenerateTimeStamp()
        {
            ...
            return Convert.ToInt64(ts.TotalSeconds + 305).ToString();
        }
        ...
        public static string GetAuthorizationHeaderForPost_OR_QueryParameterForGET(Uri url, string httpMethod, string consumerKey, string consumerSecret, string token, string tokenSecret, out string normalizedUrl, string callbackUrl = "", string bodyText = "")
        {
            ...
            if (bodyText != "")
            {
                bodyText = bodyText.Replace(" ", "%20"); // la posta de taringa lo envia con + pero si pongo + me tira error de signature
                parameters.Add("body", bodyText);
            }
                if (callbackUrl != "")
            {
