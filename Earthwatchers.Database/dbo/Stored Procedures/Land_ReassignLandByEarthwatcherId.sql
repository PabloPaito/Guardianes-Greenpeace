...
DELETE FROM EarthwatcherLands Where Land in (Select Id From Land Where GeoHexKey = @newGeoHexKey and RegionId = @RegionId)  --Borra la nueva land que se le asigno al que la tenga si es que alguno la tiene asignada(robada)
INSERT INTO EarthwatcherLands (Land, Earthwatcher) VALUES ((Select Id From Land Where GeoHexKey = @newGeoHexKey and RegionId = @RegionId), @EarthwatcherId)  --Le asigna la nueva land al usuario
Update Land set LandStatus = 2, StatusChangedDateTime = GETUTCDATE() Where GeoHexKey = @newGeoHexKey and RegionId = @RegionId   --Le pone status 2 sin revisar a la nueva land del usuario
