...
namespace Earthwatchers.Data
{
    public class PopupMessageRepository : IPopupMessageRepository
    {
        ...
        public List<PopupMessage> GetMessage()
        {
            ...
            var pms = all.Where(x => x.StartDate <= DateTime.UtcNow && x.EndDate >= DateTime.UtcNow).ToList();
            if (pms != null)
                return pms;
            else
                return null;
        ...
        public List<PopupMessage> GetAllMessages() //TODO: Agregar el pais del cual queres recibir los msjs
        {
            connection.Open();
