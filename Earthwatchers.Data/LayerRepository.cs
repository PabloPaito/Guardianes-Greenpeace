...
namespace Earthwatchers.Data
{
    public class LayerRepository : ILayerRepository
    {
        ...
        public void SaveLayer(Layer lay)
        {
            ...
            cmd.Parameters.Add(new SqlParameter("@regionId", lay.RegionId));
        ...
        public List<Zone> GetZones(int layerId, bool justBasecamps = false)
        {
            var cmd = connection.CreateCommand() as SqlCommand;
            cmd.CommandType = CommandType.StoredProcedure;
            List<Zone> zones = connection.Query<Zone>(string.Format("EXEC Layer_GetZones {0}, {1}", layerId, justBasecamps)).ToList();
        ...
        public Layer GetLayerByName(string name, bool justBasecamps = false)
        {
            connection.Open();
            var cmd = connection.CreateCommand() as SqlCommand;
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            if (layer != null)
            {
                layer.Zones = this.GetZones(layer.Id, justBasecamps);
            }
            connection.Close();
            return layer;
        }
